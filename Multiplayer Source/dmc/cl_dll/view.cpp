...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	if ( iHasNewViewAngles )
	{
		...
		//gEngfuncs.Con_NPrintf( 1, "NewAngles: %f %f %f.\n", vecNewViewAngles[0], vecNewViewAngles[1], vecNewViewAngles[2] );
	}
	...
	if (iIsSpectator == 2)
	{
		...
		// Need to interpolate angles here
		t = pparams->time - 0.1;
		...
		if ( i < ORIGIN_MASK && ViewInterp.AngleTime[ foundidx & ORIGIN_MASK ] != 0.0 )
		{
			...
			if ( dt > 0.0 )
			{
				...
				//gEngfuncs.Con_NPrintf( 3, "Frac: %f.\n", frac);
				//gEngfuncs.Con_NPrintf( 4, "Diff: %f.\n", pparams->time - ViewInterp.AngleTime[ foundidx & ORIGIN_MASK ]);
				...
				//gEngfuncs.Con_NPrintf( 3, "Frac: %f.\n", frac);
				//gEngfuncs.Con_NPrintf( 6, "From Yaw: %f.\n", ViewInterp.Angles[ foundidx & ORIGIN_MASK ][1]);
				//gEngfuncs.Con_NPrintf( 7, "To Yaw: %f.\n", ViewInterp.Angles[ (foundidx + 1) & ORIGIN_MASK ][1]);
				//gEngfuncs.Con_NPrintf( 8, "Result: %f.\n", pparams->cl_viewangles[1]);

				gEngfuncs.Con_DPrintf( "Frac: %f.\n", frac);
				gEngfuncs.Con_DPrintf( "From Yaw: %f.\n", ViewInterp.Angles[ foundidx & ORIGIN_MASK ][1]);
				gEngfuncs.Con_DPrintf( "To Yaw: %f.\n", ViewInterp.Angles[ (foundidx + 1) & ORIGIN_MASK ][1]);
				gEngfuncs.Con_DPrintf( "Result: %f.\n", pparams->cl_viewangles[1]);
	...
	if ( cl_vsmoothing && cl_vsmoothing->value &&
		( iIsSpectator || (pparams->smoothing && ( pparams->maxclients > 1 ) ) ) )
	{
		int foundidx;
		int i;
		float t;
...
void V_CalcSpectatorRefdef ( struct ref_params_s *pparams )
{
	...
	// do the sommting only once per frame
	...
	if ( pparams->nextView == 0 )
	{
		...
		switch (gHUD.m_Spectator.m_iMainMode)
		{
			case MAIN_MAP_FREE  :	pparams->onlyClientDraw = true;
									...
									gHUD.m_Spectator.GetChasePosition( angles,  gHUD.m_Spectator.m_OverviewData.origin, 4096.0f / gHUD.m_Spectator.m_OverviewData.userZoom , pparams->vieworg );
									VectorCopy ( angles, pparams->viewangles );
									break;
					
			case MAIN_MAP_CHASE :	pparams->onlyClientDraw = true;
									gHUD.m_Spectator.GetEntityPosition( 0, pparams->vieworg, pparams->viewangles );
									break;
	...
	else
	{
		...
		switch (gHUD.m_Spectator.m_iInsetMode)
		{
			...
			case INSET_NORMAL_WIDE :pparams->viewport[0] = XRES(4);	// change viewport to inset window
									pparams->viewport[1] = YRES(4);
									pparams->viewport[2] = XRES(632);
									pparams->viewport[3] = YRES(200);
									break;
		}
