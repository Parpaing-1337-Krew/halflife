...
#define MAIN_DIRECTED			4
#define MAIN_MAP_FREE			5
#define MAIN_MAP_CHASE			6

#define INSET_OFF				0
#define	INSET_NORMAL			1
#define	INSET_NORMAL_WIDE		2
...
typedef struct overviewInfo_s {
	...
	float		userZoom;	// zoom the user currently uses
...
public:
	...
	void DrawOverview();
	void GetEntityPosition(int entityIndex, float *returnvec, float * returnangle);
	void DrawOverviewEntities();
	void GetChasePosition(float * targetangles, float * target, float distance, float * returnvec);
	void DrawOverviewLayer();
	void LoadMapSprites();
	bool ParseOverviewFile();
	bool IsActivePlayer(cl_entity_t * ent);
	void SetModes(int iMainMode, int iInsetMode);
	void HandleButtons(int ButtonPressed);
